...
``data/raw`` is registered as ``raw_snapshot`` records with live sha256
fingerprints so ``gz_property_valuation catalog`` can list it without a parquet main chain.
...
# 原始证据文件（相对 raw 证据目录的路径）→ 查询/入口 URL（EVIDENCE 来源为索引记录）
...
class DatasetInfo:
    """示例城市数据集注册（gz_property_valuation catalog 展示，WP3-D 目录注册）。"""
