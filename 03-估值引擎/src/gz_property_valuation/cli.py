"""gz_property_valuation CLI (project skeleton).
...
from gz_property_valuation import __version__, catalog, config
from gz_property_valuation.contract import registry as contract_registry
from gz_property_valuation.contract.models import SubjectProperty
from gz_property_valuation.entities import alias as entities_alias
...
from gz_property_valuation.ingest.floorplan_profile import profile_floorplan
...
from gz_property_valuation.ingest.marts_build import build_combined_marts
from gz_property_valuation.ingest.profile_xlsx import profile_xlsx
from gz_property_valuation.ingest.stage import MARTS_LAYER, VALID_SALE_FILENAME, data_stage
from gz_property_valuation.reporting import backtest_report as reporting_backtest_report
from gz_property_valuation.reporting import markdown as reporting_markdown
from gz_property_valuation.reporting import run_show as reporting_run_show
...
from gz_property_valuation.valuation import (
...
def _cmd_version(args: argparse.Namespace) -> int:
    print(f"gz_property_valuation {__version__}")
    return 0
...
def _xlsx_metadata(path: Path) -> dict[str, object]:
    ...
    只读源文件，绝不修改；用于 `gz_property_valuation ingest binary` 填充原始快照 manifest 的
...
def _cmd_ingest_binary(args: argparse.Namespace) -> int:
    ...
    """
    from gz_property_valuation.ingest.xlsx_parse import PARSE_RULE_VERSION
    ...
    try:
        result = write_binary_snapshot(
            ...
            verification_ref="examples/phase2_demo/extfp0_portrait.json",
...
def _cmd_floorplan_select(args: argparse.Namespace) -> int:
    ...
    from gz_property_valuation.ingest.xlsx_stage import ORDINARY_FILENAME, read_current_run

    current = read_current_run(data_dir)
    if current is None:
        print(
            "[floorplan select] no current run pointer "
            "(staged/lianjia_ext/current.json); run gz_property_valuation xlsx stage first"
        )
        return 1
...
def _floorplan_select_production(data_dir: Path, parquet_path: Path, out_arg: Path | None) -> int:
    ...
    """
    from gz_property_valuation.ingest.floorplan_production import build_production_selection
...
def _cmd_floorplan_acceptance(args: argparse.Namespace) -> int:
    ...
    抽样结果是与 SelectionManifest 兼容的清单，可被 ``gz_property_valuation floorplan download`` 消费；
...
def _cmd_floorplan_asset(args: argparse.Namespace) -> int:
    ...
    读取 ``--run`` 目录（gz_property_valuation floorplan download 的输出：download_state.json + *.img），
...
def _cmd_floorplan_verify(args: argparse.Namespace) -> int:
    ...
    if args.repeat_annotations is not None:
        if not args.repeat_annotations.is_file():
            ...
            return 1
        from gz_property_valuation.ingest.floorplan_verify import read_annotation_table

        repeat_annotations = read_annotation_table(args.repeat_annotations)
    try:
        from gz_property_valuation.ingest.floorplan_verify import verify_run
...
def _cmd_floorplan_status(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        from gz_property_valuation.ingest.floorplan_verify import build_status_report
...
def _cmd_floorplan_freeze(args: argparse.Namespace) -> int:
    ...
    try:
        if args.verify_only is not None:
            from gz_property_valuation.ingest.data_freeze import _repo_root_of
...
def _cmd_floorplan_unrecognized(args: argparse.Namespace) -> int:
    """``gz_property_valuation floorplan`` 未带子命令时的兜底：给用法而非崩溃（RV-EXTFP2-C-01#F1）。"""
    print(
        "usage: gz_property_valuation floorplan {select,download,asset,e2e,ocr,transcribe,verify,status,freeze} "
        "[options]",
        file=sys.stderr,
    )
    return 2
...
def _cmd_xlsx_unrecognized(args: argparse.Namespace) -> int:
    """``gz_property_valuation xlsx`` 未带子命令时的兜底：给用法而非崩溃（RV-EXTFP0-D-01#F1）。"""
    print(
        "usage: gz_property_valuation xlsx {profile,floorplan,parse,stage,quality} --input <XLSX> --out <JSON>",
        file=sys.stderr,
    )
    return 2
...
def _cmd_xlsx_attributes_stage(args: argparse.Namespace) -> int:
    ...
    """
    from gz_property_valuation.ingest.xlsx_stage import stage_attributes_run
...
def _cmd_xlsx_quality(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"[xlsx quality] 无法读取 staged 表（先运行 gz_property_valuation xlsx stage）：{exc}")
        return 1
    source_sha256 = None
...
def _cmd_data_stage(args: argparse.Namespace) -> int:
    ...
    ``source/dataset@fetched_at`` forms shown by ``gz_property_valuation catalog``.
    """
    data_dir = args.data_dir or config.data_dir()
...
def _cmd_data_marts_enrich(args: argparse.Namespace) -> int:
    ...
    """
    from gz_property_valuation.ingest.attribute_enrich import enrich_attributes_mart
...
def _cmd_run_show(args: argparse.Namespace) -> int:
    """WP7-C 运行清单查询：gz_property_valuation run show <run_id>（stdout 只写 §10.3 JSON）。"""
    data_dir = args.data_dir or config.data_dir()
...
def _cmd_report_build(args: argparse.Namespace) -> int:
    """WP7-C Markdown 报告生成：gz_property_valuation report build --valuation <id>（§11.2 十二节）。
...
def _cmd_review_apply(args: argparse.Namespace) -> int:
    """WP7-D 复核留痕：gz_property_valuation review apply --valuation <id> --input <json>。
...
def _cmd_backtest_run(args: argparse.Namespace) -> int:
    """WP8-A 滚动历史回放：gz_property_valuation backtest run --config <yaml>（§13.3/§10.3/§10.4）。
...
def _cmd_backtest_report(args: argparse.Namespace) -> int:
    """WP8-B 可复现回放报告：gz_property_valuation backtest report --run <run_id>（§13.3/§12.2）。

    从回放产物只读生成 Markdown + JSON 报告（examples/phase2_demo/backtest_reports/）；
...
def _cmd_shadow_register(args: argparse.Namespace) -> int:
    """WP9-A 影子标的登记：gz_property_valuation shadow register --subject <json>（SHADOW-001）。
...
def _cmd_shadow_backfill(args: argparse.Namespace) -> int:
    """WP9-A 后续成交回填：gz_property_valuation shadow backfill（时间外，不挑选样本）。
...
def _cmd_shadow_monitor(args: argparse.Namespace) -> int:
    """WP9-A 影子误差监控：gz_property_valuation shadow monitor（近期滚动窗口 + 数据新鲜度）。
...
def build_parser() -> argparse.ArgumentParser:
    ...
    backtest_report_cmd.add_argument(
        "--out-dir",
        type=Path,
        help="replay report root (default: project examples/phase2_demo/backtest_reports/)",
    )
    ...
    xlsx_attributes_stage_cmd.add_argument(
        "--source-run-id",
        required=True,
        help="源 staged run_id（如 20200101T000000Z；只读，不改动）",
    )
    xlsx_attributes_stage_cmd.add_argument(
    ...
    # F1（RV-EXTFP0-D-01 积压）收敛：gz_property_valuation xlsx 不带子命令时给出用法，而非 AttributeError
    ...
    floorplan_asset_cmd.add_argument(
        "--run", type=Path, required=True, help="gz_property_valuation floorplan download 的输出目录"
    )
    floorplan_asset_cmd.add_argument(
        "--out",
        type=Path,
    ...
    floorplan_e2e_cmd.add_argument(
        ...
        type=Path,
        required=True,
        help="全量 selection_manifest JSON（gz_property_valuation floorplan select 输出）",
    )
    floorplan_e2e_cmd.add_argument(
        "--sample-dir",
        type=Path,
    ...
    floorplan_ocr_cmd.add_argument(
        ...
        type=Path,
        required=True,
        help="资产 manifest JSON（gz_property_valuation floorplan asset 输出的 floorplan_asset_manifest.json）",
    )
    floorplan_ocr_cmd.add_argument(
        "--out",
        type=Path,
    ...
    subparsers.add_parser("system-check", help="alias for `gz_property_valuation system check`").set_defaults(
