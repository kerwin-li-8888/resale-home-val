"""WP7-D ``gz_property_valuation review apply`` 复核命令（技术方案 §10.2/§11.1）。

对齐技术方案合同：``gz_property_valuation review apply --valuation <run_id> --input <json>``。
...
from gz_property_valuation.contract.models import ReviewEvent
from gz_property_valuation.reporting.envelope import (
    InvalidInputError,
    MissingDependencyError,
)
from gz_property_valuation.valuation import review as review_core
...
def result_id_of_run(*, run_id: str, data_dir: Path) -> str:
    ...
    if not path.is_file():
        raise MissingDependencyError(
            f"估值结果表缺失：{path}（先运行 gz_property_valuation estimate）"
        )
    rows = pq.read_table(path).to_pylist()
    match = next((r for r in rows if str(r.get("run_id")) == run_id), None)
    if match is None:
        raise MissingDependencyError(
            f"估值结果中不存在 run_id：{run_id}（先运行 gz_property_valuation estimate）"
        )
...
def apply_review_for_run(
    *,
    ...
    return review_core.append_review_events(
        ...
        notes=f"WP7-D: gz_property_valuation review apply（run {run_id}）",
    )
