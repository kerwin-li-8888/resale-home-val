"""WP7-B 端到端估值命令（REP-001 执行）：``gz_property_valuation estimate`` 编排器。
...
from gz_property_valuation.entities import (
...
from gz_property_valuation.valuation.comparable import apply_tiers_to_candidate_table
from gz_property_valuation.valuation.difference import apply_difference_adjustments
from gz_property_valuation.valuation.scope import (
    ACTIVE_SCOPE_POLICY_VERSION,
    scope_policy_filename,
)
from gz_property_valuation.valuation.time_adjustment import apply_time_adjustments
...
class EstimateOutcome:
    """一次 ``gz_property_valuation estimate`` 的结果（供 CLI 打印与测试断言）。"""
...
def load_release_decision(data_dir: Path) -> ReleaseDecision:
    ...
    决定，治理证据见发布门禁证据）才允许输出 formal。CLI 不
...
def run_estimate(
    *,
    ...
    build = build_valuation(
        ...
        notes=f"WP7-B: gz_property_valuation estimate 端到端（{__version__}）",
    )
