...
4. **近期误差滚动窗口 + 数据新鲜度（验收④）**：``monitor`` 输出近期窗口
   ...
   ``gz_property_valuation report build`` 第 12 节「后续结果区」的数据源即本模块的
    ...
    *,
    ...
    if not track_path.is_file():
        raise MissingDependencyError(
            f"影子追踪表缺失：{track_path}（先运行 gz_property_valuation shadow register）"
        )
...
def followup_markdown(
    *,
    ...
    """后续结果区 Markdown 片段（``gz_property_valuation report build`` 第 12 节的数据源）。
    ...
    if not followup_path.is_file():
        return (
            "- （暂无影子追踪记录；`gz_property_valuation shadow register` 登记标的并 "
            "`gz_property_valuation shadow backfill` 回填后生成）"
        )
