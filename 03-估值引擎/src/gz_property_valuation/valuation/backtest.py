...
宽度、状态覆盖）。命令形式 ``gz_property_valuation backtest run --config <yaml>``。
...
- **命令（验收④）**：``gz_property_valuation backtest run --config <yaml>`` 非交互、stdout 只
...
from gz_property_valuation.valuation.estimate import EstimateOutcome, run_estimate
...
def _run_isolated_estimate(
    *,
    ...
    """
    with tempfile.TemporaryDirectory(prefix="gz_property_valuation-backtest-") as td:
        tmp = Path(td)
...
class BacktestOutcome:
    """一次 ``gz_property_valuation backtest run`` 的结果（供 CLI 打印与测试断言）。"""
