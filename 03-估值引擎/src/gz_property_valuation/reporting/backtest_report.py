...
- **诚实声明（验收②）**：回放覆盖（可回放点数/校验样本数）与数据深度限制
  显式写入报告与警告；
- **落盘（合同）**：报告写 ``examples/phase2_demo/backtest_reports/<run_id>/``（示例演示
  目录约定），不触碰冻结产物。
"""
...
#: 回放报告根：项目级 ``examples/phase2_demo/backtest_reports``（示例演示目录）。
DEFAULT_BACKTEST_REPORTS_ROOT = (
    Path(__file__).resolve().parents[4] / "examples" / "phase2_demo" / "backtest_reports"
)
...
class BacktestReportResult:
    """一次 ``gz_property_valuation backtest report`` 的结果（供 CLI 打印与测试断言）。"""
