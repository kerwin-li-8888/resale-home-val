...
- 从 staged 普通住宅表按 §10.2 维度确定性分层抽样：区县、成交年份、居室数三组
  ...
  全部冻结；
- 抽样结果输出为与 ``SelectionManifest`` 兼容的子类清单，可被 ``gz_property_valuation floorplan download``
  直接消费；
...
staged 普通住宅 parquet 与全量 selection_manifest；真实 300 张下载由 ``gz_property_valuation floorplan
download`` + ``gz_property_valuation floorplan asset`` 在显式 CLI 运行中完成。
"""
...
from gz_property_valuation.ingest.floorplan_transcribe import classify_room
...
class AcceptanceSelectionManifest(SelectionManifest):
    ...
    继承 ``SelectionManifest`` 的全部字段，可被 ``gz_property_valuation floorplan download`` 直接消费
