...
from gz_property_valuation.contract.models import MissingSemantics
from gz_property_valuation.ingest.clean import CleanedSale, CleaningSummary, SaleRecord
...
class QualityReport:
    ...
    excluded_out_of_region: list[str] = field(default_factory=list)  # norm-v2 分册
    blocked_aliases: list[str] = field(default_factory=list)  # norm-v2 分册
    district_blocked: list[str] = field(default_factory=list)  # ext 区县守门分册
...
def build_quality_report(
    ...
    *,
    ...
    excluded_out_of_region: Sequence[str] = (),
    blocked_aliases: Sequence[str] = (),
    district_blocked: Sequence[str] = (),
) -> QualityReport:
    ...
    return QualityReport(
        ...
        excluded_out_of_region=list(excluded_out_of_region),
        blocked_aliases=list(blocked_aliases),
        district_blocked=list(district_blocked),
        source_recency_days=(
...
def report_to_dict(report: QualityReport) -> dict[str, object]:
    ...
    return {
        ...
        "excluded_out_of_region": report.excluded_out_of_region,
        "blocked_aliases": report.blocked_aliases,
        "district_blocked": report.district_blocked,
...
def report_to_markdown(report: QualityReport) -> str:
    ...
    if not report.unmatched_conflicts and not report.blocked_aliases:
        lines.append(
            "- 当前为单来源（链家）数据，无跨来源实体匹配，冲突清单不适用；实体合并归 WP5。"
        )
    else:
        for item in report.unmatched_conflicts:
            lines.append(f"- 未匹配：{item}")
        for item in report.blocked_aliases:
            lines.append(f"- 低置信（待定/冲突/排除别名，不自动合并）：{item}")
    if report.excluded_out_of_region:
        n_excluded = len(report.excluded_out_of_region)
        lines.append(f"- 外区括注排除（整行不入池，单独登记）：{n_excluded} 名")
        for item in report.excluded_out_of_region:
            lines.append(f"- 外区括注排除：{item}")
    if report.district_blocked:
        lines.append(f"- 区县守门排除（整行不入池，单独登记）：{len(report.district_blocked)} 名")
        for item in report.district_blocked:
            lines.append(f"- 区县守门排除：{item}")
    lines += [
        "",
        "## 7. 未来日期与时序矛盾",
        "",
