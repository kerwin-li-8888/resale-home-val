    ...
    return XlsxQualityReport(
        ...
        rollback={
            ...
            "data_artifacts": {
                ...
                "staged_sale_record": (
                    "<data-dir>/staged/lianjia_ext/runs/<run_id>/lianjia_ext_sale_record.parquet"
                    "（不可变 run 产物，指针 current.json 切换；非 git 内容）"
                ),
                "staged_ordinary": (
                    "<data-dir>/staged/lianjia_ext/runs/<run_id>/"
            ...
            or (
                ...
                "gz_property_valuation xlsx stage 重建（源 XLSX 只读不变）。"
            ),
        },
    )
...
def report_to_markdown(report: XlsxQualityReport) -> str:
    ...
    lines += [
        ...
        "- 本报告数字全部由数据区 staged/ 机器产物统计生成，Markdown 不维护独立数字。",
        "",
    ]
    return "\n".join(lines)
...
def load_staged_tables(
    ...
    from gz_property_valuation.ingest.xlsx_stage import read_current_run

    current = read_current_run(data_dir)
    if current is None:
        raise FileNotFoundError(
            "no current run pointer (staged/lianjia_ext/current.json); run gz_property_valuation xlsx stage first"
        )
