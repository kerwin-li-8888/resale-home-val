...
把已完成的 lianjia_ext 外源数据录入线产物登记为只读可消费数据版本
...
def collect_freezable_assets(data_dir: Path) -> FreezeManifest:
    ...
    v: list[FreezeEntry] = [
        _entry(
            "debug10_samples",
            repo_root,
            "examples/phase2_demo/floorplan_samples",
            "dir",
            note="10 张调试样本 + 来源清单",
        ),
        _entry(
    ...
    sections["reports"] = [
        _entry(
            "extfp4_portrait",
            repo_root,
            "examples/phase2_demo/extfp4_portrait.json",
            "file",
        ),
        _entry(
            "extfp4_archive",
...
_PORTRAIT_REL = "examples/phase2_demo/extfp4_portrait.json"
...
> 生成方式：`gz_property_valuation floorplan freeze` 机器产物；本报告只解释，动态数字源自
...
- 目录现状披露：生产 OCR run 目录含 300 个验收集旧响应文件残留（64-hex 命名）
  与 `ocr_run.previous_20000101T000000Z.json`（验收集 run 备份）；
   OCRNEXT-D 空目录 `run_floorplan-ocr-ocrnext-d-subset-20000101T000005Z` 按现状登记

## 4. 待接入状态

- 外源数据录入线（EXTFP0—EXTFP5）：**已冻结 v1，待估值接入验证**
