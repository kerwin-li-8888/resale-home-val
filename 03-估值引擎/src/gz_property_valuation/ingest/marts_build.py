...
from gz_property_valuation.ingest.ext_source import (
    ...
    run_fetched_at,
    split_resolvable_with_district_gate,
)
...
from gz_property_valuation.ingest.parsers.lianjia_html import parse_lianjia_csv_table
from gz_property_valuation.ingest.quality import build_quality_report, write_quality_report
...
#: 来源 = `sources/链家补数探测-V0.1.md` §2（7 补数映射）+
#: §3（链家 27 条成交小区候选名录映射）+ `候选小区名录-V0.1.md`（标准 ID）。
#: P0 五小区（ext-sale-ingest-scope-v1-2）：来源 = `sources/同名核验
#: 证据包-P0五小区-V0.1.md` §0 判定表 + §8 复核记录（唯一复核人 user
#: 2026-08-31 逐条「确认」，ext run 指纹 + census 双管线）。
#: 探测报告标注「待核」（示例小区171/示例小区221/澜庭泊府等）、「名录外」或
#: 证据包未确认的相似命名（示例小区145/示例小区217/示例小区147/示例小区009等）
...
class CombinedMartResult:
    ...
    ext_district_blocked_rows: int = 0
    ext_district_blocked_names: int = 0
...
def lianjia_extended_lookup(lookup: CommunityIdLookup) -> CommunityIdLookup:
    """把链家成交社区注册表合并进回填查找表**别名补充层**（D7；仅补条目，不覆盖既有）。

    norm-v2-valuation-ingest D7：注入层位由 canonical（v1.1 现状）降位为
    「一致别名 + 注册表补充层」，位于子区层与 blocked 检查之后——「示例小区130A区」
    等子区注册名先被子区层命中拿到 ``sub_area``；「示例小区136拾光里」等命中
    merged 实体键的统一经 redirect 转发。一致别名命中优先、注册表键不覆盖
    既有条目（溯源理由 = 链家补数探测 §2/§3 + 候选名录）。返回新的
    :class:`CommunityIdLookup`，不修改入参（实体表只读）。
    """
    if not LIANJIA_COMMUNITY_REGISTRY:
        return lookup
    registry = dict(lookup.alias_registry)
    reason = "链家成交社区注册表（链家补数探测-V0.1 §2/§3 + 候选小区名录）"
    for name, community_id in LIANJIA_COMMUNITY_REGISTRY.items():
        c = cmp_norm(name)
        if not c or c in lookup.alias_consistent or c in registry:
            continue
        registry[c] = {community_id: reason}
    return CommunityIdLookup(
        canonical=lookup.canonical,
        alias_consistent=lookup.alias_consistent,
        blocked=lookup.blocked,
        subarea=lookup.subarea,
...
def build_combined_marts(*, data_dir: Path) -> CombinedMartResult:
    ...
    ext_excluded_counter: Counter[str] = Counter()
    ext_district_blocked_counter: Counter[str] = Counter()
    ext_input_count = 0
    if ext_run is not None:
        ...
        # 区县守门在清洗与跨源去重之前（expand-lianjia-alias-rebaseline D1）：
        # 解析命中但区县明确非目标区的行独立成册，不入池、不参与清洗与去重。
        kept, ext_unmatched_counter, ext_excluded_counter, ext_district_blocked_counter = (
            split_resolvable_with_district_gate(
                ext_records, extract_districts(ext_run.table), community_lookup
            )
        )
    ...
    if ext_run is not None:
        notes += (
            ...
            f" ext_excluded_rows={sum(ext_excluded_counter.values())}"
            f" ext_excluded_names={len(ext_excluded_counter)}"
            f" ext_district_blocked_rows={sum(ext_district_blocked_counter.values())}"
            f" ext_district_blocked_names={len(ext_district_blocked_counter)}"
    ...
    # 未匹配/低置信/外区括注排除 三分册登记（跨源回填后仍无法命中标准身份的，
    # 不静默归并）。ext 未解析行未入合并表，其代表性源名按册并入（LJ-E 如实标记）。
    unresolved = classify_unresolved(
        merged_sale.column("community").to_pylist(), community_lookup
    )
    unmatched = unresolved["unmatched"]
    blocked_aliases = unresolved["blocked"]
    excluded_out_of_region = unresolved["excluded_out_of_region"]
    if ext_unmatched_counter:
        unmatched = sorted(set(unmatched) | set(ext_unmatched_counter))
    if ext_excluded_counter:
        excluded_out_of_region = sorted(
            set(excluded_out_of_region) | set(ext_excluded_counter)
        )
    ...
    report = build_quality_report(
        ...
        excluded_out_of_region=excluded_out_of_region,
        blocked_aliases=blocked_aliases,
        district_blocked=sorted(ext_district_blocked_counter),
    )
    ...
    return CombinedMartResult(
        ...
        ext_district_blocked_rows=sum(ext_district_blocked_counter.values()),
        ext_district_blocked_names=len(ext_district_blocked_counter),
