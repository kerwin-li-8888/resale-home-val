...
Consumes WP4-B :class:`~gz_property_valuation.ingest.parsers.lianjia.LianjiaRecord`
...
from gz_property_valuation.entities.backfill import CommunityIdLookup, resolve_community_id
from gz_property_valuation.ingest.manifests import (
    DerivedManifest,
    InputRef,
    write_derived_manifest,
)
from gz_property_valuation.ingest.parsers.fang_esf import FangEsfRecord
...
def sale_event_table(
    ...
    *,
    ...
    def _identity(community: str | None) -> tuple[str | None, str | None]:
        """(community_id, sub_area)：provisional 保留 + norm-v2 子区回填。"""
        if not apply_lookup:
            return community, None
        assert community_lookup is not None  # guarded by apply_lookup (mypy narrowing)
        return resolve_community_id(community, community_lookup)

    for sale in cleaned:
        r = sale.record
        fetched = fetched_at.replace(tzinfo=UTC) if fetched_at.tzinfo is None else fetched_at
        identity = _identity(r.community)
        row_values: dict[str, object] = {
            ...
            "community_id": identity[0],  # WP5-E backfill（norm-v2）
            "sub_area": (
                identity[1] if identity[1] is not None else MissingSemantics.UNKNOWN.value
            ),
            "layout": r.layout,
