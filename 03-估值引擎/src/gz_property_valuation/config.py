...
environment variable is renamed from ``PHILLY_DATA_DIR`` to ``GZV_DATA_DIR``.
...
def data_dir() -> Path:
    ...
    Defaults to ./data; override with the GZV_DATA_DIR environment variable.
    """
    return Path(os.environ.get("GZV_DATA_DIR", str(DEFAULT_DATA_DIR)))
...
def evidence_dir() -> Path:
    ...
    captured by WP2/WP3-A (PNG/TXT/HTML under the raw evidence dir), registered as
    ``raw_snapshot`` records by the data contract without a parquet main chain.
    Defaults to the repository's raw evidence directory regardless of the working
    directory; override with the GZV_EVIDENCE_DIR environment variable.
    """
    default = Path(__file__).resolve().parents[3] / "examples" / "demo_raw"
    return Path(os.environ.get("GZV_EVIDENCE_DIR", str(default)))