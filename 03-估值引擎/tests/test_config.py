"""Project configuration: default data-lake root and the GZV_DATA_DIR override."""
...
from gz_property_valuation import config
...
def test_default_data_dir_is_relative_data(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("GZV_DATA_DIR", raising=False)
...
def test_env_override_points_at_custom_root(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("GZV_DATA_DIR", str(Path("D:/custom-lake")))
...
def test_data_dir_used_by_snapshot_writer(
    ...
    monkeypatch.setenv("GZV_DATA_DIR", str(tmp_path))
...
def test_evidence_dir_points_at_repo_raw_dir() -> None:
    ...
    assert evidence.name == "demo_raw"
    assert (evidence.parent.name) == "examples"


def test_evidence_dir_env_override(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("GZV_EVIDENCE_DIR", str(Path("D:/custom-evidence")))
